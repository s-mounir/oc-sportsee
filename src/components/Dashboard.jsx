...
function Dashboard(props) {
    ...
    const getWidth = useContainerDimensions(ref,width,setWidth);
    ...
    return (
        <div>
                    <div>
                        <Bonjour>Bonjour <FNSpan>{response.user.firstName}</FNSpan></Bonjour>
                        <HeaderText>Félicitation ! Vous avez explosé vos objectifs hier 👏</HeaderText>
                    </div>
                    <DashboardDiv ref={ref}>
                        <BarChart data={response.activity} width={barChartWidth}/>
                        <LineChart data={response.sessions} width={columnWidth} />
                        <RadarChart data={response.performance} width={columnWidth} />
                        <RadialBarChart data={response.user.score} width={columnWidth}/>
                        <DashboardCards>
                            <InfoCard label='Calories' data={response.user.keyData.calorieCount}>
                            ...
                            <InfoCard label='Proteines' data={response.user.keyData.proteinCount}>
                                <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <InfoCard label='Glucides' data={response.user.keyData.carbohydrateCount}>
                                <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <InfoCard label='Lipides' data={response.user.keyData.lipidCount}>
                                <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
